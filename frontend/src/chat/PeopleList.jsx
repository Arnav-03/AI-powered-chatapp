      ...
      {Object.values(People).map((user) => (
        ...
        >
          <div className="flex items-center gap-4 w-full ">
            <div className={`uppercase border-2  rounded-full w-12 h-12 text-center flex items-center justify-center  text-2xl 
            ${user._id===selectedUser?"border-[#f0efec]":" border-[#1f1f1f]"}`}>{user.username[0]}</div>
            <div className="nextf1  text-xl ">{user.username}</div>
          </div>
